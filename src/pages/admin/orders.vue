...
const header: any = [
  ...
  },
  {
    name: "product_id",
    required: true,
    label: "Buyurtma berilgan tovar(ID)",
    field: "product_id",
    align: "left",
    sortable: true,
  },
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <div class="bg-white w-[230px]">
          <q-input
            v-model="orders_data.filters.search"
            outlined
            dense
            placeholder="Qidiruv(ID)..."
        ...
        <div class="bg-white w-[230px]">
          <q-select
            v-model="orders_data.filters.status.value"
            :options="orders_data.filters.status.data"
            option-value="value"
            option-label="title"
            outlined
            dense
      ...
      <div class="flex items-center justify-end">
        <q-pagination
          v-model="orders_data.pagination.page"
          color="primary"
          :max="orders_data.pagination.max"
          :max-pages="orders_data.pagination.max_pages"
          boundary-numbers
        />
      </div>
    </div>
          ...
          <td>
            <div class="flex items-center gap-4">
              ...
              >
                ...
                <q-tooltip> Buyurtma berilgan tovarni ko'rish </q-tooltip>
              </q-btn>

              <div>ID: {{ props.row.product_id }}</div>
            </div>
          </td>
        </template>
