...
<template>
  <div class="min-h-screen">
    ...
    <div class="flex items-center justify-between">
      <div class="">
        <span class="font-semibold">{{ $t("products.search") }}</span>
        {{
          route.query.search ? route.query.search : route.query.category_name
        }}
      </div>
      <div>
        <q-pagination
          v-model="products_data.pagination.page"
          color="primary"
          :max="products_data.pagination.max"
          :max-pages="products_data.pagination.max_pages"
          boundary-numbers
          unelevated
    ...
    <div class="mb-4">
      <div
        class="grid lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-3 max-[630px]:grid-cols-2 gap-4 mt-4"
        v-if="products_data.products.length"
      >
        <div
          class="bg-white rounded-md p-4 card cursor-pointer"
          v-for="product of products_data.products"
          :index="product.id"
          @click="
            $router.push({
              path: '/product',
        ...
        >
          <div class="!w-full">
            <img
              :src="CreateUrl(product.images[0].name)"
              :alt="product.name"
          ...
          <div>
            ...
            <div class="mt-4 font-semibold">
              {{
                parseInt(product.price).toLocaleString().split(",").join(" ")
              }}
              UZS
            </div>
          </div>
